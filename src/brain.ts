...
export class Brain {
    ...
    public foward(input: number[]): number[][] {
        let layers: number[][] = new Array(this.biases.length + 1)
        layers[0] = input
        for (let l = 0; l < (this.biases).length; l++) {
            layers[l + 1] = this.biases[l].slice()
            ...
            layers[l + 1] = layers[l + 1].map(x => mathFuncs(x, this.activationFunctions[l], false))
        }
        return layers
    }
    ...
    public backprop(layers: number[][], loss: number[]): [number[][], number[][][]] {
        var bd: number[][] = new Array(this.biases.length)
        var wd: number[][][] = new Array(this.weights.length)
        for (let l = this.biases.length - 1; l >= 0; l--) {

            bd[l] = this.biases[l].map((_, n) =>
                loss[n] * mathFuncs(layers[l + 1][n], this.activationFunctions[l], true)
            )
            wd[l] = this.weights[l].map((_, i) =>
                bd[l].map((g) => layers[l][i] * g)
            )
            loss = layers[l].map((_, i) =>
                loss.reduce((s, value, j) =>
                    s + value * this.weights[l][i][j], 0)
            )
        }
